...
import { bankTransactions, slips, lineIdentities, messages, conversations } from "@joeai/db";
import {
  LineClient,
  createLogger,
  getEnv,
...
const log = createLogger("worker.slip-matching");

function startOfBangkokDay(d: Date): Date {
  // Start of Bangkok day = previous UTC 17:00 (UTC+7)
...
export async function handleSlipMatching(job: SlipMatchingJob): Promise<void> {
  ...
  const [slip] = await db.select().from(slips).where(eq(slips.id, job.slipId)).limit(1);
  ...
  // Find candidate transactions: same amount, within ±1 day window
  ...
  if (outcome.status === "matched_auto") {
    ...
    if (!claimed) {
      ...
      // Re-queue itself — another slip won this transaction
...
async function claimTransaction(txnId: string, slipId: string): Promise<boolean> {
...
async function sendReply(
  ...
  // Find the LINE userId for the slip's customer
  const [row] = await db
    .select({
      lineUserId: lineIdentities.lineUserId,
    })
    .from(slips)
